...
# Count movements and calculate pushback times
for index, row in df_movements.iterrows():
    ...
    if parking_pos in parking_to_index and runway in runway_to_index:
        ...
        # Check if it's a pushback event and calculate pushback time
        if row['isPushback']:
            pushback_time = (row['startTaxi'] - row['startPushback']).total_seconds()
            ...
            time_count[0] += pushback_time
            time_count[1] += 1

# Calculate average pushback times and store in matrices
